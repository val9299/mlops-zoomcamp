...
def run_train(
    ...
    # Log `mse` to Weights & Biases under the key `"MSE"` - added by Valerie G.
    wandb.log({"MSE": mse})

    with open("regressor.pkl", "wb") as f:
        pickle.dump(rf, f)

    # Log `regressor.pkl` as an artifact of type `model` - added by Valerie G.
    model_artifact = wandb.Artifact(f"regression-model", type="model")
